...
Для того, чтобы использовать фотометр по полной программе, вам необходимо объединиться с другими людьми. Для этого, нужно или создать новую группу, или присоединиться к уже существующей.\n
...
Ты можешь это сделать перейдя по вот этой ссылке: [{event}]({}{})",
    ...
    fn write_late_publication_mail( &self,
        ...
        let subject = format!( "{}", event_name );
        let mail = format!(
"Ну что, **{}**, не получилось вовремя опубликовать свою фотографию? Ну ничего, не растраивайся!

Ты всё равно можешь это сделать, пройдя вот [по этой ссылке]({}). Возможно она уже не будет участвовать в конкурсах,
но зато никто не подумает что ты лентяй ;).",
            user_name,
            events::make_event_link( scheduled_id )
        );
        ( subject, mail )
